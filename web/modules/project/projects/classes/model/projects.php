...
class Model_Projects {
    ...
    public function add($data = array()) {
        $result = DB::query(Database::INSERT, 'INSERT INTO ' . $this->tableName . ' (alias, weight, is_fine, status) VALUES (:alias, :weight, :is_fine, :status)')
                ->parameters(array(
    ...
    public function edit($Id, $data = array()) {
        DB::query(Database::UPDATE, 'UPDATE ' . $this->tableName . ' SET `alias` = :alias, `weight` = :weight, is_fine = :is_fine, `status` = :status WHERE `id` = :id')
                ->parameters(array(
                    ':id' => $Id,                    
                    ':alias' => Security::xss_clean($data['alias']), 
                    ':weight' => Security::xss_clean($data['weight']),
    ...
    public function get_content($id = '') {
        ...
        if (count($result)>0){
			...
			$files = $file_obj->get_files_by_content_id($result[0]['id'], 'projects');
			if (empty($files)) {
				$filename = false;
			} else {
			...
			$edit_interface = Liteedit::get_interface($result[0]['id'], 'projects', 'teaser', true);
    ...
    public function get_total($cat = 0, $is_adminka = 0) {
		if($cat){
			if ($is_adminka) {
				$sql = "SELECT COUNT(id) AS total FROM `" . $this->tableName2 . "` cc INNER JOIN " . $this->tableName . " a ON cc.content_id = a.id  WHERE cc.category_id = " . $cat . " AND cc.module = 'projects'";"SELECT COUNT(id) AS total FROM " . $this->tableName;
			} else {
				$sql = "SELECT COUNT(id) AS total FROM `" . $this->tableName2 . "` cc INNER JOIN " . $this->tableName . " a ON cc.content_id = a.id  WHERE cc.category_id = " . $cat . " AND cc.module = 'projects' AND a.status = 1";
			}
		} else {
			if ($is_adminka) {
