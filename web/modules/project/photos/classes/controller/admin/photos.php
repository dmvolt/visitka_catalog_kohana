...
class Controller_Admin_Photos extends Controller_Admin_Template {
    ...
    public function action_index() {
		...
		if ($catid) {
			$total = $photos_obj->get_total($catid, 1);   // Получение общего количества записей
			...
			$contents = $photos_obj->get_all_to_cat($catid, $result['start'], $result['num'], 'a.weight', 1);

			$cat_name = "Статьи - " . $cats[$catid]['name'];
			
		} else {
			$total = $photos_obj->get_total(0, 1);   // Получение общего количества записей
			$result = Pagination::start($total);
			$pagination = Pagination::admin_navigation($result['page'], $total, $result['total_page'], $result['num']);
			$pagination2 = Pagination::admin_navigation2($result['page'], $total, $result['total_page'], $result['num']);
			$contents = $photos_obj->get_all(1, $result['start'], $result['num'], 'a.weight');
        ...
        if (isset($_POST['descriptions'][1]['title'])) {
            ...
            if ($validation->check()) {
                ...
                if ($new_content_id) {
                    ...
                    Request::initial()->redirect('admin/photos'.$this->parameters);
                }
            }
