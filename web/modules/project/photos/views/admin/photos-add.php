<? if (isset($errors)) { ?>
    ...
    <? } ?>
<? } ?>
<h2 class="title"><?= $text_add_new_photos ?></h2>
...
<div id="tabs">
	...
	<div id="tabs-1">
		...
		<div class="form_item_textarea">
			<?php if(isset($languages)): ?>
				<?php foreach ($languages as $item): ?>
					<label for="body" class="lang_img_<?= $item['lang_id']?>"><?= $text_body ?>(Видео) - Для вставки видео скопируйте ссылку с Youtube вставте в окно редактора и нажмите Enter </label><?= $item['icon']?><textarea name="descriptions[<?= $item['lang_id']?>][body]" class="textarea_lang_<?= $item['lang_id']?>"><?= $post['descriptions'][$item['lang_id']]['body'] ?></textarea>
				<?php endforeach; ?>
			<?php else: ?>
				<label for="body"><?= $text_body ?>(Видео) - Для вставки видео скопируйте ссылку с Youtube вставте в окно редактора и нажмите Enter </label><textarea name="descriptions[1][body]" class="lang_editor" id="editor2"><?= $post['descriptions'][1]['body'] ?></textarea>
			<?php endif; ?>
		</div>
		<div class="form_item">
		...
		<div class="form_item" style="top:35px">
			<label for="status"><?= $text_photos_status ?></label><input type="checkbox" id="status" name="status" checked="checked" value="1">
		</div>
	</div>
