...
class Model_Catalog {
    ...
    public function add($data = array()) {
        $result = DB::query(Database::INSERT, 'INSERT INTO ' . $this->tableName . ' (alias, `price1`, weight, is_fine, status) VALUES (:alias, :price1, :weight, :is_fine, :status)')
                ->parameters(array(
                    ':alias' => Security::xss_clean($data['alias']),  
					':price1' => Security::xss_clean($data['price1']), 					
    ...
    public function edit($Id, $data = array()) {
        DB::query(Database::UPDATE, 'UPDATE ' . $this->tableName . ' SET `alias` = :alias, `price1` = :price1, `weight` = :weight, is_fine = :is_fine, `status` = :status WHERE `id` = :id')
                ->parameters(array(
                    ':id' => $Id,                    
                    ':alias' => Security::xss_clean($data['alias']), 
					':price1' => Security::xss_clean($data['price1']),
	...
	public function get_all_no_fine($is_adminka = 0, $start = 0, $num = 100, $field = 'id') {
		$contents = array();
        if ($is_adminka) {
            $sql = "SELECT * FROM " . $this->tableName . " WHERE is_fine = 0 ORDER BY `weight` LIMIT :start, :num";
        } else {
            $sql = "SELECT * FROM " . $this->tableName . " WHERE is_fine = 0 AND `status` = 1 ORDER BY `weight` DESC LIMIT :start, :num";
        } 
        $result = DB::query(Database::SELECT, $sql)
                ->parameters(array(
                    ':field' => $field, 
	...
	public function get_all_is_fine($is_adminka = 0, $start = 0, $num = 100, $field = 'id') {
		$contents = array();
        if ($is_adminka) {
            $sql = "SELECT * FROM " . $this->tableName . " WHERE is_fine = 1 ORDER BY `weight` LIMIT :start, :num";
        } else {
            $sql = "SELECT * FROM " . $this->tableName . " WHERE is_fine = 1 AND `status` = 1 ORDER BY `weight` DESC LIMIT :start, :num";
        } 
        $result = DB::query(Database::SELECT, $sql)
                ->parameters(array(
                    ':field' => $field, 
    ...
    public function get_all_to_cat($cat, $start = 0, $num = 100, $field = 'a.weight', $is_adminka = 0) {
	
        $contents = array();
		
		if ($is_adminka) {
            $sql = 'SELECT * FROM `' . $this->tableName2 . '` cc INNER JOIN ' . $this->tableName . ' a ON cc.content_id = a.id  WHERE cc.category_id = :category_id AND cc.module = :module ORDER BY ' . $field . ' LIMIT :start, :num';
        } else {
            $sql = 'SELECT * FROM `' . $this->tableName2 . '` cc INNER JOIN ' . $this->tableName . ' a ON cc.content_id = a.id  WHERE cc.category_id = :category_id AND cc.module = :module AND a.status = 1 ORDER BY ' . $field . ' LIMIT :start, :num';
        } 
    ...
    public function get_content($id = '') {
        ...
        if (count($result)>0){
			...
			$files = $file_obj->get_files_by_content_id($result[0]['id'], 'catalog');
			if (empty($files)) {
				$filename = false;
			} else {
			...
			$edit_interface = Liteedit::get_interface($result[0]['id'], 'catalog', 'teaser', true);
			
			$contents = array(
				'id' => $result[0]['id'],
				'descriptions' => $descriptions,
				'price1' => $result[0]['price1'],
    ...
    public function get_total($cat = 0, $is_adminka = 0) {
		if($cat){
			if ($is_adminka) {
				$sql = "SELECT COUNT(id) AS total FROM `" . $this->tableName2 . "` cc INNER JOIN " . $this->tableName . " a ON cc.content_id = a.id  WHERE cc.category_id = " . $cat . " AND cc.module = 'catalog'";"SELECT COUNT(id) AS total FROM " . $this->tableName;
			} else {
				$sql = "SELECT COUNT(id) AS total FROM `" . $this->tableName2 . "` cc INNER JOIN " . $this->tableName . " a ON cc.content_id = a.id  WHERE cc.category_id = " . $cat . " AND cc.module = 'catalog' AND a.status = 1";
			}
		} else {
			if ($is_adminka) {
