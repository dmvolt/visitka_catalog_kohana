<? if (isset($errors)) { ?>
    ...
    <? } ?>
<? } ?>
<h2 class="title"><?= $text_add_new_news ?></h2>
...
<div id="tabs">
	...
	<div id="tabs-1">
		...
		<div class="form_item">
            <label for="type"><?= $text_news_type ?></label><input type="checkbox" name="type" value="1">
        </div>
		
		<div class="form_item_textarea">
			<?php if(isset($languages)): ?>
				<?php foreach ($languages as $item): ?>
					<label for="poll" class="lang_img_<?= $item['lang_id']?>"><?= $text_news_poll ?> </label><?= $item['icon']?><textarea name="descriptions[<?= $item['lang_id']?>][poll]" class="seo_textarea_lang_<?= $item['lang_id']?> text" style="width:400px; height:150px; display:block"><?= $post['descriptions'][$item['lang_id']]['poll'] ?></textarea>
				<?php endforeach; ?>
			<?php else: ?>
				<label for="poll"><?= $text_news_poll ?></label><textarea name="descriptions[1][poll]" class="text" style="width:400px; height:150px; display:block"><?= $post['descriptions'][1]['poll'] ?></textarea>
			<?php endif; ?>
        </div>
        ...
        <div class="form_item_textarea">
			<?php if(isset($languages)): ?>
				...
				<?php endforeach; ?>
			<?php else: ?>			
				<label for="teaser"><?= $text_teaser ?></label><textarea name="descriptions[1][teaser]" class="lang_editor" id="editor1"><?= $post['descriptions'][1]['teaser'] ?></textarea></br>
			<?php endif; ?>
        </div>
        <div class="form_item_textarea">
			<?php if(isset($languages)): ?>
        ...
        <div class="form_item" style="top:35px">
            <label for="status"><?= $text_news_status ?></label><input type="checkbox" id="status" name="status" checked="checked" value="1">
        </div>
	</div>
