<? if (isset($errors)) { ?>
    ...
    <? } ?>
<? } ?>
<?= Modulinfo::get_admin_block('reviews') ?>
<h2 class="title"><?= $text_add_new_reviews ?></h2>
<form method="post" action="" id="form_add_reviews">
	<?//= $categories_form ?>
	...
	<div class="form_item" style="top:32px;">
		<a onclick="$('#form_add_reviews').submit();" class="btn_core btn_core_blue btn_core_md"><span><?= $text_create ?></span></a>
	</div>
	<br>
	<div class="form_item">
	...
	<div class="form_item_textarea">
		<?php if(isset($languages)): ?>
			<?php foreach ($languages as $item): ?>
				<label for="body" class="lang_img_<?= $item['lang_id']?>"><?= $text_body_reviews ?> </label><?= $item['icon']?><br class="lang_img_<?= $item['lang_id']?>"><textarea name="descriptions[<?= $item['lang_id']?>][body]" class="textarea_lang_<?= $item['lang_id']?>"><?= $post['descriptions'][$item['lang_id']]['body'] ?></textarea>
			<?php endforeach; ?>
		<?php else: ?>
			<label for="body"><?= $text_body_reviews ?></label><br><textarea name="descriptions[1][body]" class="lang_editor" id="editor1"><?= $post['descriptions'][1]['body'] ?></textarea>
		<?php endif; ?>
	</div>
</form>
...
<?php if (isset($contents) and count($contents) > 0): ?>
    <form method="post" action="/admin/weight/update" id="weight_form">
		<!--<h2><?//= $group_reviews[2][Data::_('lang_id')]['name'] ?></h2>-->
		<table>
			<thead>
				<tr>
					<td><strong>ID отзыва</strong></td>
					<td><strong><?= $text_reviews_thead_date ?></strong></td>
					<td><strong><?= $text_reviews_thead_name ?></strong></td>
					<td class="last"><strong><?= $text_reviews_thead_status ?></strong>&nbsp;&nbsp;<a onclick="$('#weight_form').submit();" class="btn_core btn_core_blue btn_core_sm"><span><?= $text_save ?></span></a></td>
					...
					<td class="last"><strong><?= $text_reviews_thead_action ?></strong></td>
				</tr>
			</thead>
			<tbody>
				<?php foreach ($contents as $value): ?>
					<tr>
						...
						<td class="last"><a href="/admin/reviews/edit/<?= $value['id'] ?>" class="edit"><?= $text_edit ?></a>&nbsp;&nbsp;<a href="/admin/reviews/delete/<?= $value['id'] ?>" class="delete"><?= $text_delete_img ?></a></td>
					</tr>
					<?php if ($value['answer']): ?>
						<?php foreach ($value['answer'] as $value2): ?>
							<tr style="background:#a4f6cd;">
								...
								<td class="last"><a href="/admin/reviews/edit/<?= $value2['id'] ?>" class="edit"><?= $text_edit ?></a>&nbsp;&nbsp;<a href="/admin/reviews/delete/<?= $value2['id'] ?>" class="delete"><?= $text_delete_img ?></a></td>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
				<?php endforeach; ?>
