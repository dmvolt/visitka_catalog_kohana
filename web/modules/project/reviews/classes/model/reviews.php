...
class Model_Reviews {
    ...
    public function add($data = array()) {
        DB::query(Database::INSERT, 'INSERT INTO ' . $this->tableName . ' (date, parent_id, group_id, weight, status) VALUES (:date, :parent_id, :group_id, :weight, :status)')
				->parameters(array(		
    ...
    public function edit($Id, $data = array()) {
        DB::query(Database::UPDATE, 'UPDATE ' . $this->tableName . ' SET `date` = :date, `group_id` = :group_id, `weight` = :weight, `status` = :status WHERE `id` = :id')
				->parameters(array(
					':id' => $Id, 		
					':date' => Security::xss_clean($data['date']),
					':group_id' => $data['group_id'],
    ...
    public function get_content($id = '') {
        ...
        if ($result){
			...
			$files = $file_obj->get_files_by_content_id($id, 'reviews');
			if (empty($files)) {
				$filename = false;
			} else {
			...
			$edit_interface = Liteedit::get_interface($id, 'reviews', 'teaser', true);
